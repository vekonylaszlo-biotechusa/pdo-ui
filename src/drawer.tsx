...
function DrawerBackdrop({
  ...
  return (
    <DrawerPrimitive.Backdrop
      className={cn(
        'fixed inset-0 z-50 bg-backdrop-accent opacity-[calc(1-var(--drawer-swipe-progress))] backdrop-blur-(--panel-blur) transition-opacity duration-normal ease-in-out data-ending-style:opacity-0 data-starting-style:opacity-0 data-ending-style:duration-[calc(var(--drawer-swipe-strength)*400ms)] data-swiping:duration-0 supports-[-webkit-touch-callout:none]:absolute',
        className,
      )}
          ...
          className={cn(
            ...
            variant !== 'straight' &&
              cn(
                ...
                position === 'top' &&
                  'rounded-b-md **:data-[slot=drawer-footer]:rounded-b-[calc(var(--radius-md)-1px)]',
                position === 'left' &&
                  'rounded-e-2xl **:data-[slot=drawer-footer]:rounded-ee-[calc(var(--radius-md)-1px)]',
                position === 'right' &&
                  'rounded-s-2xl **:data-[slot=drawer-footer]:rounded-es-[calc(var(--radius-md)-1px)]',
              ),
            variant === 'default' &&
              cn(
                position === 'bottom' &&
                  'before:rounded-t-[calc(var(--radius-md)-1px)]',
                position === 'top' &&
                  'before:rounded-b-[calc(var(--radius-md)-1px)]',
                position === 'left' &&
                  'before:rounded-e-[calc(var(--radius-md)-1px)]',
                position === 'right' &&
                  'before:rounded-s-[calc(var(--radius-md)-1px)]',
              ),
            variant === 'inset' &&
              'before:hidden sm:rounded-md sm:border sm:after:bg-transparent sm:before:rounded-[calc(var(--radius-2xl)-1px)] sm:**:data-[slot=drawer-footer]:rounded-b-[calc(var(--radius-2xl)-1px)]',
